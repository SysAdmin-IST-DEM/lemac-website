...
export default {
  ...
  async mounted() {
    ...
    const sortedWorkstationData = new Map(
      [...workstation_data.entries()].sort((a, b) => a[0] - b[0])
    );
    ...
    this.count_by_hour_data = {
      type: 'line',
      data: {
        labels: time_data.map(
          (entry) =>
            `${entry.time_interval.toLocaleTimeString('pt-PT', { hour: '2-digit', minute: '2-digit', second: '2-digit' })}`
        ),
        datasets: [
          {
            label: 'Total number of users in pc',
