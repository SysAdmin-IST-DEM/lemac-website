  ...
  export default {
    ...
    methods: {
      ...
      async loadWorkstations () {
        ...
        for (const x of available) workstationsSorted.push(x)
        workstationsSorted.push({ type: 'subheader', name: 'Partly Available' })
        for (const x of partlyAvailable) workstationsSorted.push(x)
        workstationsSorted.push({ type: 'subheader', name: 'Not Available' })
        for (const x of notAvailable) workstationsSorted.push({ ...x, disabled: 'true' })
      ...
      async editEntry (entry) {
        ...
        this.editingEntry.number = this.editingEntry.istId.split('ist1', 2)[1]
      },
    },
  }
</script>
