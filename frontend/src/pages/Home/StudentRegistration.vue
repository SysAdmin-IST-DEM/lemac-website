<template>
      ...
      <v-col>
        <div>
          <div class="mx-auto text-center mt-4">
            ...
            <h2 class="text-xl">Room Entrance and Other Services</h2>
          </div>
        </div>
      </v-col>
    </v-row>
  ...
  >
    <v-card>
      ...
      >
        <p>Thank you for your registration in the LEMAC system.</p>

        <p><b>Registered Student:</b> {{ student }}</p>

        <div v-if="$route.query.isDEM === 'false'">
          <p class="mb-0"><b class="text-error">Attention:</b> You are not an active student, teacher or researcher in one of
            these courses:</p>
          ...
          <p>Most services provided by LEMAC are allowed to all IST students, but you might find some
            restrictions by not being registered as a DEM student.</p>

          <p>You might also not be permitted to access the LEMAC laboratory room.</p>
        </div>

        <div v-else-if="$route.query.mifare === 'true'">
          <p>You can now use your student card to access the LEMAC laboratory room.</p>
        </div>

        <div v-else>
          <p class="mb-0"><b class="text-error">Action required:</b></p>
          <p class="mb-0">Your student card is not yet registered in the system due to an error validating your card. To finish the process:</p>

          <ol class="mb-4">
            <li>1. Go to the LEMAC room.</li>
            <li>2. Locate the Monitor on duty near the entrance.</li>
            <li>3. Ask to have your card manually assigned to your profile.</li>
          </ol>
        </div>
      ...
      <v-card-text
        ...
        class="registration-text"
      >
        <p>There was an error during the registration process.</p>
